...
namespace Lombiq.DataTables.Liquid
{
    public class ActionsLiquidFilter : ILiquidFilter
    {
        ...
        {
            var title = arguments.HasNamed("title") ? arguments["title"].ToStringValue() :
                arguments.Count > 0 ? arguments.At(0).ToStringValue() :
                T["Actions"];

            return input?.Type switch
            {
                FluidValues.String => FromObject(new ActionsModel { Id = input.ToStringValue() }, title),
                FluidValues.Object => input.ToObjectValue() switch
                {
                    ActionsModel model => FromObject(model, title),
                    JToken jToken => FromObject(jToken.ToObject<ActionsModel>(), title),
        ...
        private ValueTask<FluidValue> FromObject(ActionsModel model, string title)
        {
            ...
            foreach (var menuItem in model.GetAllMenuItems(_hca.HttpContext, _linkGenerator, T))
            {
